...
14. **「ルールの作成」** ページで **「+ 条件の追加」** を選択し、ドロップダウン メニューから **「コンテンツが Microsoft 365 から共有されている」** を選択します。

15. **「Microsoft 365 からコンテンツを共有する」** セクションで **「組織内の連絡先のみ」** オプションを選択します（既定では選択されています）。

16. **「ルールの作成」** ページで **「+ アクションの追加」** を選択し、 **「Microsoft 365 の場所にあるコンテンツへのアクセスを制限またはコンテンツを暗号化する」** を選択します。
...
19. **ルールの作成** ページの **ユーザーによる上書き** セクションで、**M365 サービスによる上書きを許可する** の下にある **M365 サービスによる上書を許可します。Power BI、Exchange、SharePoint、OneDrive、Teams のユーザーがポリシー制限を上書きするのを許可します** ボックスをオンにします

    > **注:** **M365 サービスからの上書きを許可する** のチェック ボックスをオンにできなかった場合は、前の手順の **ユーザー通知 >>  Microsoft 365 サービス** の下にある **Office 365 のユーザーにポリシーのヒントを通知する** をオンにします。 
...
4. **「設定」** ページの **「Information Protection/情報保護」** で **「ファイル」** を選択します。
