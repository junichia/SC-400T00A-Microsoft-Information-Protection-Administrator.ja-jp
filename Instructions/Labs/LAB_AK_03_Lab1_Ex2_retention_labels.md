...
8. **「保持設定の定義」** ページの **「アイテムを無期限にまたは特定の期間保持する」** 設定を有効にします。
...
11. 「**保持期間後の動作を選択する**」ページで「**保持設定を無効にする**」を選択します。
    
10. **[次へ]** ボタンを選択します。

11. **「確認と完了」** ページで、 **「ラベルの作成」** ボタンを選択します。  *[保持ラベルが作成されました]* ページで、 **[何も行わない]** オプションを選択して、 **[完了]** を選択します。

12. **[データ ライフサイクル管理]** ページの **[ラベル]** タブに戻ります。ラベルは演習の後半で発行します。
...
16. **[保持設定の定義]** ページの **[アイテムを無期限にまたは特定の期間保持する]** 設定がオンになっていることを確認します。 ドロップダウン リストから **[カスタム]** を選択し、次の情報を設定します。
    - **保持期間**:3 年
    - **保持期間開始の条件**:アイテムが作成されたとき

17. **[次へ]** ボタンを選択します。

1. 「**保持期間後の動作を選択する**」ページで「**保持設定を無効にする**」を選択します。
...
17. **[ポリシーを適用する場所の選択]** ページで、次のオプションを **[オン]** に切り替えます。    
...
4. マウスの右ボタンを使って、中心のウィンドウで 1 つ目のメール アイテムを選択し、メニューで **「Assign Policy/ポリシーの割り当て」** を選択します。
...
4. SharePoint が開いたら、下にスクロールして SharePoint サイト **「Communication Site」** を選択します。     
   ...
   > 検索バーに「**Communication Site**」と入力し、それを検索します。
